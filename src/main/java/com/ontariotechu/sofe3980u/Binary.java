...
public class Binary
{
	...
	*/
	 ...
	 */
	public static void or(Binary num1,Binary num2) {
	 ...
	 * @param num2 The second binary operand
	 */
	public static void and(Binary num1, Binary num2) {
		String a = num1.getValue();
		String b = num2.getValue();
		...
		for (int i = 0; i < maxLen; i++) {
			...
			char bitA = (idxA >= 0) ? a.charAt(idxA) : '0';
			char bitB = (idxB >= 0) ? b.charAt(idxB) : '0';

			result.insert(0, (bitA == '1' && bitB == '1') ? '1' : '0');
		}
	 ...
	 */
	public static void multiply(Binary num1,Binary num2) {
		...
		for (int i = b.length() - 1; i >= 0; i--) {
			if (b.charAt(i) == '1') {
				...
				result = Binary.add(result, shifted); // static add
			}
			shift++;
		}

		System.out.println("MULTIPLY result: " + result.getValue());
	}
}	
